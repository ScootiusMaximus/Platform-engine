    ...
    def tick_enemies(self):
        for chunk in self.loaded_chunks:
            if chunk is not None:
                eToDel = []
                for mob in chunk.enemyEntities:
                    if not mob.needsDel:
                        mob.fix_center()
                        mob.tick()
                        mob.update_hitboxes()
                ...
                jToDel = []
                for mob in chunk.jumpingEnemyEntities:
                    if not mob.needsDel:
                        mob.fix_center()
                        mob.update_target((self.player.xpos,self.player.ypos))
                    ...
                    if mob.state == 1 and mob.name == "final" and mob.summon:
                        ...
                        if c == 1:
                            ...
                            for _ in range(10):
                                ...
                                self.bombEntities.append(Bomb(mob.lastStanding[0] + pos[0],
                                                           mob.lastStanding[1] + pos[1], self.img.image["bomb"],
                                                           gravity=self.gravity))
                        else:
                            for _ in range(5):
                                posMod = make_position_modifier(0, 2)
                                self.enemyEntities.append(Enemy(mob.lastStanding[0] + posMod[0],
                ...
                for mob in eToDel:
                    ...
                    chunk.enemyEntities.remove(mob)
                for mob in jToDel:
                    self.stats.enemiesKilled += 1
                    self.make_death_particle(mob)
                    chunk.jumpingEnemyEntities.remove(mob)
                for mob in bToDel:
                    self.stats.enemiesKilled += 1
                    self.stats.bossesKilled += 1
                    chunk.bossEntities.remove(mob)
...
##        self.player.rect = pygame.Rect(self.player.xpos-25,self.player.ypos-25,50,50)
    ...
    def handle_platform_collision_new(self):
        for which in self.entities:
            for mob in which:
                if mob.xvel != 0 or mob.yvel != 0:
        ...
        for item in self.platforms:
            compItem = toRect(item)
            for which in self.entities:
                for mob in which:
                    if mob.xvel != 0 or mob.yvel != 0:
                        if collide(mob.hitbox.bottom, compItem):
                            mob.wallData[0] = True
                            mob.hitbox.collideBottom = item
                            mob.floorMaterial = "platform"
                            # print("yup")
                        if collide(mob.hitbox.left, compItem):
                            mob.wallData[1] = True
                            mob.hitbox.collideLeft = item
                        if collide(mob.hitbox.right, compItem):
                            mob.wallData[2] = True
                            mob.hitbox.collideRight = item
                        if collide(mob.hitbox.top, compItem):
                            mob.wallData[3] = True
                            mob.hitbox.collideTop = item
                        if collide(mob.hitbox.whole, compItem):
                            mob.wallData[4] = True
        ...
        for i in range(len(self.belts)):
            compItem = toRect([self.belts[i][0],self.belts[i][1],50,50])
            for which in self.entities:
                for mob in which:
                    if mob.xvel != 0 or mob.yvel != 0:
                        if collide(mob.hitbox.bottom, compItem):
    ...
    def handle_platform_collision_OG(self):
        for which in self.entities:
            for mob in which:
                mob.floorMaterial = None
    ...
    def handle_platform_collision(self):
        ...
        for chunk in self.loaded_chunks:
            if chunk is not None:
                self.player = self.check_chunk_platforms(self.player,chunk)
                for i in range(len(chunk.enemyEntities)):
                    chunk.enemyEntities[i] = self.check_chunk_platforms(chunk.enemyEntities[i],chunk)
                for i in range(len(chunk.jumpingEnemyEntities)):
                    chunk.jumpingEnemyEntities[i] = self.check_chunk_platforms(chunk.jumpingEnemyEntities[i], chunk)
                for i in range(len(chunk.bossEntities)):
                    chunk.bossEntities[i] = self.check_chunk_platforms(chunk.bossEntities[i], chunk)
                for i in range(len(chunk.bombEntities)):
                    chunk.bombEntities[i] = self.check_chunk_platforms(chunk.bombEntities[i], chunk)

        if True in self.player.wallData:
            self.sound.end_fall()

    def check_chunk_platforms(self,mob,chunk):
        collide = pygame.Rect.colliderect

        for item in chunk.platforms:
            compItem = toRect(get_actual_pos(item))
            if collide(mob.hitbox.actBottom, compItem):
                mob.wallData[0] = True
                mob.hitbox.collideBottom = item
        ...
        for i in range(len(chunk.disappearingPlatforms)):
            plat = chunk.disappearingPlatforms[i]
            compItem = toRect(get_actual_pos(plat))
            if not self.buttonPresses[chunk.disappearingPlatformLinks[i]]:
                if collide(mob.hitbox.actBottom, compItem):
                    mob.wallData[0] = True
                    mob.hitbox.collideBottom = plat
                    mob.floorMaterial = "disappearing platform"
        ...
        for i in range(len(chunk.appearingPlatforms)):
            plat = chunk.appearingPlatforms[i]
            compItem = toRect(get_actual_pos(plat))
            if self.buttonPresses[chunk.appearingPlatformLinks[i]]:
                if collide(mob.hitbox.actBottom, compItem):
                    mob.wallData[0] = True
                    mob.hitbox.collideBottom = plat
                    mob.floorMaterial = "appearing platform"
        ...
        for i in range(len(chunk.belts)):
            compItem = toRect(get_actual_pos([chunk.belts[i][0], chunk.belts[i][1], 50, 50]))
            if collide(mob.hitbox.actBottom, compItem):
                mob.wallData[0] = True
                mob.hitbox.collideBottom = [chunk.belts[i][0], chunk.belts[i][1], 50, 50]
                mob.floorMaterial = "belt"
                mob.xpos += self.misc.beltSpeed * self.beltDirections[i]
            if collide(mob.hitbox.actLeft, compItem):
                mob.wallData[1] = True
                mob.hitbox.collideLeft = [chunk.belts[i][0], chunk.belts[i][1], 50, 50]
            if collide(mob.hitbox.actRight, compItem):
                mob.wallData[2] = True
                mob.hitbox.collideRight = [chunk.belts[i][0], chunk.belts[i][1], 50, 50]
            if collide(mob.hitbox.actTop, compItem):
                mob.wallData[3] = True
                mob.hitbox.collideTop = [chunk.belts[i][0], chunk.belts[i][1], 50, 50]
            if collide(mob.hitbox.actWhole, compItem):
                mob.wallData[4] = True
                mob.hitbox.collideWhole = [chunk.belts[i][0], chunk.belts[i][1], 50, 50]
                mob.floorMaterial = "belt"
    ...
    def handle_spike_collision(self):
        for chunk in self.loaded_chunks:
            if chunk is not None:
                spikeRects = []
                for item in chunk.spikes:
                    orn = chunk.spikeDir[chunk.spikes.index(item)]
                    ...
                    for which in [chunk.enemyEntities,chunk.jumpingEnemyEntities]:
                        for mob in which:
                ...
                for item in chunk.electricHitboxes:
                ...
                for mob in chunk.bossEntities:
                    if not mob.needsDel and mob.name in ["fireball"]:
                        mob.check_vision(chunk.spikes)

    def handle_player_enemy_collision(self):
        for chunk in self.loaded_chunks:
            if chunk is not None:
                for which in [chunk.bossEntities, chunk.enemyEntities, chunk.jumpingEnemyEntities]:
                    for mob in which:
                        if pygame.Rect.colliderect(mob.hitbox.actWhole,self.player.hitbox.actWhole):
                            self.player.isDead = True
                            self.player.deathCause = "enemy"
                            #self.trigger_death()
                            if which == chunk.bossEntities and self.settings.annoyingBosses:
                                self.end()

    def handle_misc_item_effects(self):
        for chunk in self.chunks:
            if chunk is not None:
                for item in chunk.checkpoints:
                ...
                for item in chunk.buttons:
                    ...
                    if True in press:
                        chunk.buttonPresses[self.buttons.index(item)] = True

    def handle_bomb_entities(self):
        for chunk in self.loaded_chunks:
            if chunk is not None:
                for item in chunk.bombEntities:
                    item.tick()
                    item.update_hitbox()
    ...
    def tick(self):
        ...
        self.time(self.tick_bombs,"tick_bombs")
        #self.handle_scene()
        self.time(self.handle_platform_collision,"handle_platform_collision")
        self.time(self.handle_spike_collision,"handle_spike_collision")
        self.time(self.handle_player_enemy_collision,"handle_player_enemy_collision")
        self.time(self.handle_misc_item_effects,"handle_misc_item_effects")
        self.time(self.handle_scene,"handle_scene") # temp
    ...
    def update_level(self,next=False):
        ...
        for i in range(len(level["mobs"])):
            ...
            try:
                ...
                resist = "none"
            self.enemyEntities.append(self.make_enemy(item[0] + 20, item[1],resistType=resist,maxXvel=random.randint(5, 6)))
        ...
        for item in level["bosses"]:
            ...
            if self.levelIDX >= 30:
                ...
                self.bossEntities.append(Final_Boss(item[0]+50,item[1]+50,imgs=[self.img.image["boss_img"],self.img.image["fireball"]],health=health))
    ...
    def create_chunks(self):
        ...
        yposes_min = []
        yposes_max = []
        for which in [self.platforms, self.spikes, self.fanBases, self.fanColumns, self.stars,
                      self.enemies, self.jumpingEnemies, self.checkpoints, self.bosses, self.buttons,
                      self.disappearingPlatforms, self.appearingPlatforms, self.bombs, self.ice,
                      self.saws, self.electric, self.lights, self.nc_plats, self.windows, self.belts]:
            for item in which:
                xposes_min.append(item[0])
                yposes_min.append(item[1])
                if len(item) == 4:
                    xposes_max.append(item[0] + item[2])
                    yposes_max.append(item[1] + item[3])

        min_x = min(min(xposes_min), min(xposes_max))
        max_x = max(max(xposes_min), max(xposes_max))
        min_y = min(min(yposes_min), min(yposes_max))
        max_y = max(max(yposes_min), max(yposes_max))
        self.misc.levelRect = [min_x,min_y,max_x-min_x,max_y-min_y]

        self.chunks.clear()
        self.num_chunks_x = (int(max_x - min_x) // (CHUNKSIZE*50)) + 2
        self.num_chunks_y = (int(max_y - min_y) // (CHUNKSIZE*50)) + 2
        for x in range(self.num_chunks_x):
            for y in range(self.num_chunks_y):
                chonk = self._chunk_maker(x+(min_x//(50*CHUNKSIZE)),y+(min_y//(50*CHUNKSIZE)))
                self.chunks.append(chonk)

    def _chunk_maker(self,x,y):
        platforms = []
        spikes = []
        spikeDir = []
        fanBases = []
        ...
        disappearingPlatformLinks = []
        appearingPlatforms = []
        appearingPlatformLinks = []
        bombs = []
        bombEntities = []
        ice = []
        iceStates = []
        ...
        chunkRect = (x*CHUNKSIZE*50,y*CHUNKSIZE*50,50*CHUNKSIZE,50*CHUNKSIZE)
        box = toRect(chunkRect)
        for item in self.platforms:
            plat = toRect(item)
            if pygame.Rect.colliderect(plat,box):
                if box.contains(plat):
                    platforms.append(plat)
                else:
                    platforms.append(box.clip(plat))
        for i in range(len(self.spikes)):
            if box.contains(toRect(self.spikes[i])):
                spikes.append(self.spikes[i])
                spikeDir.append(self.spikeDir[i])
        for item in self.fanBases:
            if box.contains(toRect(item)):
                fanBases.append(item)
        for item in self.fanColumns:
            if box.contains(toRect(item)):
                fanColumns.append(item)
        ...
        for i in range(len(self.buttons)):
            if box.contains(toRect(self.buttons[i])):
                buttons.append(self.buttons[i])
                buttonPresses.append(self.buttonPresses[i])
        for i in range(len(self.disappearingPlatforms)):
            plat = toRect(self.disappearingPlatforms[i])
            if pygame.Rect.colliderect(plat, box):
                if box.contains(plat):
                    disappearingPlatforms.append(plat)
                else:
                    disappearingPlatforms.append(box.clip(plat))
                disappearingPlatformLinks.append(self.disappearingPlatformLinks[i])
        for i in range(len(self.appearingPlatforms)):
            plat = toRect(self.appearingPlatforms[i])
            if pygame.Rect.colliderect(plat, box):
                if box.contains(plat):
                    appearingPlatforms.append(plat)
                else:
                    appearingPlatforms.append(box.clip(plat))
                appearingPlatformLinks.append(self.appearingPlatformLinks[i])
        for i in range(len(self.ice)):
            plat = toRect(self.ice[i])
            if pygame.Rect.colliderect(plat, box):
                if box.contains(plat):
                    ice.append(plat)
                else:
                    ice.append(box.clip(plat))
                iceStates.append(self.iceStates[i])
        for item in self.saws:
            if box.contains(toRect(item)):
                saws.append(item)
        for i in range(len(self.electric)):
            if box.contains(toRect(self.electric[i])):
                electric.append(self.electric[i])
                electricStates.append(self.electricStates[i])
                electricHitboxes.append(self.electricHitboxes[i])
        for i in range(len(self.lights)):
            if box.contains(toRect(self.lights[i])):
                lights.append(self.lights[i])
                lightEntities.append(self.lightEntities[i])
        for i in range(len(self.nc_plats)):
            plat = toRect(self.nc_plats[i])
            if pygame.Rect.colliderect(plat, box):
                if box.contains(plat):
                    nc_plats.append(plat)
                else:
                    nc_plats.append(box.clip(plat))
        for item in self.windows:
            if box.contains(toRect(item)):
                windows.append(item)
        for i in range(len(self.belts)):
            if box.contains(toRect(self.belts[i])):
                belts.append(self.belts[i])
                beltStates.append(self.beltStates[i])
                beltDirections.append(self.beltDirections[i])
        for i in range(len(self.enemies)):
            if box.contains(toRect(self.enemies[i])):
                enemies.append(self.enemies[i])
                enemyEntities.append(self.enemyEntities[i])
        for i in range(len(self.jumpingEnemies)):
            if box.contains(toRect(self.jumpingEnemies[i])):
                jumpingEnemies.append(self.jumpingEnemies[i])
                jumpingEnemyEntities.append(self.jumpingEnemyEntities[i])
        for i in range(len(self.bosses)):
            if box.contains(toRect(self.bosses[i])):
                bosses.append(self.bosses[i])
                bossEntities.append(self.bossEntities[i])
        for i in range(len(self.bombs)):
            if box.contains(toRect(self.bombs[i])):
                bombs.append(self.bombs[i])
                bombEntities.append(self.bombEntities[i])
        ...
        return Chunk(
            ...
            jumpingEnemies=jumpingEnemies,
            jumpingEnemyEntities=jumpingEnemyEntities,
            ...
            disappearingPlatformLinks=disappearingPlatformLinks,
            appearingPlatforms=appearingPlatforms,
            appearingPlatformLinks=appearingPlatformLinks,
            bombs=bombs,
            bombEntities=bombEntities,
            ice=ice,
            iceStates=iceStates,
            saws=saws,
            electric=electric,
            electricStates=electricStates,
            electricHitboxes=electricHitboxes,
            lights=lights,
    ...
    def tick_animated_textures(self):
    ...
    def draw_bg(self):
        #sendToCam(self.misc.levelRect, "hitbox", colour.white)
        #for chunk in self.chunks:
        #    sendToCam(chunk.rect, "hitbox", colour.white)

        for chunk in self.loaded_chunks:
            if chunk is not None:
                #sendToCam(chunk.rect, "hitbox", colour.red)

                for item in chunk.nc_plats:
                    sendPlatformToCam(item, self.settings.highResTextures, platType="background")
                for item in chunk.windows:
                ...
                for i in range(len(chunk.ice)):
                    if not chunk.iceStates[i]:
                        sendPlatformToCam(chunk.ice[i],self.settings.highResTextures,col=self.misc.platformCol,platType="ice")
                for item in chunk.saws:
                    blitToCam(self.img.image["saw"][self.misc.sawState], item)
                for i in range(len(chunk.electric)):
                    sendElectricToCam(chunk.electric[i],self.misc.electricState,chunk.electricStates[i])
                for i in range(len(chunk.belts)):
                    sendBeltToCam(chunk.belts[i],chunk.beltDirections[i],chunk.beltStates[i],self.misc.beltState)
                    #sendBeltToCam(self.belts[i],0,self.beltStates[i],self.misc.beltState)

                if self.scene == "ingame":
                    for i in range(len(chunk.buttons)):
                        if chunk.buttonPresses[i]:
                            blitToCam(self.img.image["button_pressed"], chunk.buttons[i])
                        else:
                            blitToCam(self.img.image["button_unpressed"],chunk.buttons[i])

                    for i in range(len(chunk.disappearingPlatforms)):
                        idx = self.disappearingPlatformLinks[i] # get which button it is linked to
                        #print(f"idx {idx}")
                        if idx != -1:
                            if not self.buttonPresses[idx]:
                                sendPlatformToCam(chunk.disappearingPlatforms[i],self.settings.highResTextures,col=(0,25,80),platType="disappearing")

                    for i in range(len(chunk.appearingPlatforms)):
                        idx = self.appearingPlatformLinks[i]
                        if idx != -1:
                            if self.buttonPresses[idx]:
                                sendPlatformToCam(chunk.appearingPlatforms[i],self.settings.highResTextures,col=(100,150,221),platType="appearing")
                ...
                # NOT CHANGED FOR CHUNKS YET
                if self.scene == "editor":
                    for i in range(len(chunk.enemies)):
                        item = chunk.enemies[i]
                    ...
                    for item in chunk.jumpingEnemies:
                        blitToCam(self.img.image["jumping_enemy_for_editor"],(item[0]+5,item[1]+5))

                    for item in chunk.bosses:
                        blitToCam(self.img.image["boss_img"],(item[0]-250,item[1]-250))

                    for item in chunk.buttons:
                        blitToCam(self.img.image["button_unpressed"], item)

                    for item in chunk.disappearingPlatforms:
                        sendPlatformToCam(item,self.settings.highResTextures,platType="disappearing")#, col=[80,80,100])

                    for item in chunk.appearingPlatforms:
                        sendPlatformToCam(item,self.settings.highResTextures,platType="appearing")#, col=[180,180,200])

                    for item in chunk.bombs:
                        blitToCam(self.img.image["bomb"],item)

                for item in chunk.lights:
                    blitToCam(self.img.image["light"],item)

        blitToCam(self.img.image["finish"], self.data[str(self.levelIDX)]["end"])

    def get_active_chunks(self,target_x,target_y):
        x = target_x // (CHUNKSIZE * 50)
        y = target_y // (CHUNKSIZE * 50)

        center_chunk = None
        chunkIDX = 0
        for i in range(len(self.chunks)):
            if self.chunks[i].pos == (x, y):
                center_chunk = self.chunks[i]
                chunkIDX = i
                break

        chunks = [center_chunk]
        for each in [chunkIDX + 1,chunkIDX - 1,chunkIDX - self.num_chunks_y,chunkIDX - self.num_chunks_y - 1,
                     chunkIDX - self.num_chunks_y + 1,chunkIDX + self.num_chunks_y,chunkIDX + self.num_chunks_y - 1,
                     chunkIDX + self.num_chunks_y + 1]:
            try:
                chunks.append(self.chunks[each])
            except IndexError:
                pass
        return chunks

    def update_chunks(self):
        self.loaded_chunks = self.get_active_chunks(self.player.xpos, self.player.ypos)

    def switch_chunks_for_entities(self):
        for chunk in self.loaded_chunks:
            if chunk is not None and len(chunk.enemyEntities) != 0:
                entities = chunk.enemyEntities.copy()
                for i in range(len(chunk.enemyEntities)):
                    mob = entities[i]
                    pos = (mob.xpos // (CHUNKSIZE*50),
                           mob.ypos // (CHUNKSIZE*50))
                    # establish which chunk this is
                    if chunk.pos == pos:
                        continue
                    for j in range(len(self.loaded_chunks)):
                        if self.loaded_chunks[j] is chunk:
                            continue
                        if self.loaded_chunks[j].pos == pos:
                            self.loaded_chunks[j].enemyEntities.append(chunk.enemyEntities.pop(i))
                            continue
                entities = chunk.jumpingEnemyEntities.copy()
                for i in range(len(chunk.jumpingEnemyEntities)):
                    mob = entities[i]
                    pos = (mob.xpos // (CHUNKSIZE * 50),
                           mob.ypos // (CHUNKSIZE * 50))
                    ...
                    for j in range(len(self.loaded_chunks)):
                        if self.loaded_chunks[j] is chunk:
                            continue
                        if self.loaded_chunks[j].pos == pos:
                            self.loaded_chunks[j].jumpingEnemyEntities.append(chunk.jumpingEnemyEntities.pop(i))
                            continue
                entities = chunk.bossEntities.copy()
                for i in range(len(chunk.bossEntities)):
                    mob = entities[i]
                    pos = (mob.xpos // (CHUNKSIZE * 50),
                           mob.ypos // (CHUNKSIZE * 50))
                    ...
                    for j in range(len(self.loaded_chunks)):
                        if self.loaded_chunks[j] is chunk:
                            continue
                        if self.loaded_chunks[j].pos == pos:
                            self.loaded_chunks[j].bossEntities.append(chunk.bossEntities.pop(i))
                            continue
                entities = chunk.bombEntities.copy()
                for i in range(len(chunk.bombEntities)):
                    mob = entities[i]
                    pos = (mob.xpos // (CHUNKSIZE * 50),
                           mob.ypos // (CHUNKSIZE * 50))
                    ...
                    for j in range(len(self.loaded_chunks)):
                        if self.loaded_chunks[j] is chunk:
                            continue
                        if self.loaded_chunks[j].pos == pos:
                            self.loaded_chunks[j].bombEntities.append(chunk.bombEntities.pop(i))
                            continue
    ...
    def check_story_update(self):
        if self.levelIDX == 30:
            ...
            self.enemyEntities.append(self.misc.helpie)
    ...
    def time(self,method,message=""):
        ...
        self.log(f"{end-start} for {message}")

    def sort_log(self):
        exprs = ["check_story", "camerashake.tick", "draw_gradient", "generate_cloud", "tick_enemies",
               "run_story_events", "correct_mobs", "tick_player", "player.update_hitboxes",
               "handle_bomb_entities", "draw_bg", "draw_enemies", "animations", "lighting","tick_bombs",
               "handle_platform_collision","handle_spike_collision","handle_player_enemy_collision",
               "handle_misc_item_effects","handle_scene"]
        sorted_log = {}
        for exp in exprs:
            sorted_log[exp] = []
        ...
        for message in self.logs:
            for exp in exprs:
                if exp in message:
                    bits = message.split("for")
                    sorted_log[exp].append(int(bits[0]))
                    break

        self.logs = []
        for key in sorted_log:
            try:
                self.logs.append(f"{mean(sorted_log[key])} for {key}")
            except:
                pass
...
class Chunk:
    def __init__(self,pos,rect,**kwargs):
        self.pos = pos
        self.rect = rect
        self.col = colour.randCol()
        self.platforms = []
        self.spikes = []
        self.spikeDir = []
        self.fanBases = []
        ...
        if "spikes" in kwargs:
            self.spikes = kwargs["spikes"]
        if "spikeDir" in kwargs:
            self.spikeDir = kwargs["spikeDir"]
        if "fanBases" in kwargs:
            self.fanBases = kwargs["fanBases"]
        if "fanColumns" in kwargs:
            self.fanColumns = kwargs["fanColumns"]
        if "enemies" in kwargs:
            self.enemies = kwargs["enemies"]
        if "enemyEntities" in kwargs:
            self.enemyEntities = kwargs["enemyEntities"]
        if "jumpingEnemies" in kwargs:
            self.jumpingEnemies = kwargs["jumpingEnemies"]
        if "jumpingEnemyEntities" in kwargs:
            self.jumpingEnemyEntities = kwargs["jumpingEnemyEntities"]
        if "stars" in kwargs:
            self.stars = kwargs["stars"]
        if "bosses" in kwargs:
            self.bosses = kwargs["bosses"]
        if "bossEntities" in kwargs:
            self.bossEntities = kwargs["bossEntities"]
        if "platforms" in kwargs:
            self.platforms = kwargs["platforms"]
        if "buttons" in kwargs:
            self.buttons = kwargs["buttons"]
        if "buttonPresses" in kwargs:
            self.buttonPresses = kwargs["buttonPresses"]
        if "disappearingPlatforms" in kwargs:
            self.disappearingPlatforms = kwargs["disappearingPlatforms"]
        if "disappearingPlatformLinks" in kwargs:
            self.disappearingPlatformLinks = kwargs["disappearingPlatformLinks"]
        if "appearingPlatforms" in kwargs:
            self.appearingPlatforms = kwargs["appearingPlatforms"]
        if "appearingPlatformLinks" in kwargs:
            self.appearingPlatformLinks = kwargs["appearingPlatformLinks"]
        if "checkpoints" in kwargs:
            self.checkpoints = kwargs["checkpoints"]
        if "bombs" in kwargs:
            self.bombs = kwargs["bombs"]
        if "bombEntities" in kwargs:
            self.bombEntities = kwargs["bombEntities"]
        if "ice" in kwargs:
            self.ice = kwargs["ice"]
        if "iceStates" in kwargs:
            self.iceStates = kwargs["iceStates"]
        if "saws" in kwargs:
            self.saws = kwargs["saws"]
        if "electric" in kwargs:
            self.electric = kwargs["electric"]
        if "electricStates" in kwargs:
            self.electricStates = kwargs["electricStates"]
        if "electricHitboxes" in kwargs:
            self.electricHitboxes = kwargs["electricHitboxes"]
        if "entities" in kwargs:
            self.entities = kwargs["entities"]
        if "lights" in kwargs:
            self.lights = kwargs["lights"]
        if "lightEntities" in kwargs:
            self.lightEntities = kwargs["lightEntities"]
        if "nc_plats" in kwargs:
            self.nc_plats = kwargs["nc_plats"]
        if "windows" in kwargs:
            self.windows = kwargs["windows"]
        if "belts" in kwargs:
            self.belts = kwargs["belts"]
        if "beltStates" in kwargs:
            self.beltStates = kwargs["beltStates"]
        if "beltDirections" in kwargs:
            self.beltDirections = kwargs["beltDirections"]
        if "family" in kwargs:
...
class Bomb(Physics_Object):
    ...
    def update_hitbox(self):
        self.hitbox.whole = toRect([self.xpos+5,self.ypos+5,40,40])
        self.hitbox.actWhole = toRect(get_actual_pos(self.hitbox.whole))
        self.hitbox.bottom = toRect([self.xpos+5,self.ypos+40,40,10])
        self.hitbox.actBottom = toRect(get_actual_pos(self.hitbox.bottom))
...
while True:
    ...
    game.log(game.scene)
    game.time(pygame.display.flip,"display.flip")
    ...
    if game.scene == "init":
        ...
        game.update_chunks()
        game.time(check_story,"check_story")
        game.time(game.camerashake.tick,"camerashake.tick")
        game.time(game.draw_gradient,"draw_gradient")
        game.time(game.generate_cloud,"generate_cloud")
        game.time(game.tick_enemies,"tick_enemies")
        game.time(game.switch_chunks_for_entities,"switch_chunks_for_entities")
        game.time(game.run_story_events,"run_story_events")
        game.time(game.tick,"tick")
        game.time(game.correct_mobs,"correct_mobs")
        game.time(game.tick_player,"tick_player")
        game.time(game.player.update_hitboxes,"player.update_hitboxes")
        ...
        game.time(game.handle_bomb_entities,"handle_bomb_entities")
        game.tick_animated_textures()
        game.time(game.draw_bg,"draw_bg")
        game.time(game.draw_enemies,"draw_enemies")
        ...
        game.time(game.draw_animations,"animations")
        game.time(game.draw_lighting,"lighting")
